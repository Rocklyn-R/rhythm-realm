...
export const RefineSearch: React.FC<RefineSearchProps> = ({ products, subcategoryName }) => {
    ...
    useEffect(() => {
        const manufacturersFetch = async () => {
            let result;
            if (!isSale) {
                result = await getManufacturers(subcategoryName);
                if (result) {
                    setManufacturers(result);
                }
            }
            if (isSale) {
                result = await getFeaturedItemManufacturers("On Sale");
                if (result) {
                    setManufacturers(result);
                }
            }
    ...
    }, [dispatch, subcategoryName, products, isSale]);
    ...
    useEffect(() => {
        const fetchCategories = async () => {
            const result = await getFeaturedCategories("On Sale");
            if (result) {
    ...
    useEffect(() => {
        const fetchSubcategories = async () => {
            ...
            if (selectedCategories.length > 0) {
                  result = await getFeaturedSubcategories("On Sale", selectedCategories);
            } else {
    ...
    return (
        <div className="w-1/4 p-4 bg-white rounded-md shadow-lg">
            <h4 className="text-xl font-bold mb-6">Refine Your Search</h4>
            {isSale && (
